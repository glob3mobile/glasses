...
public class OrientationManager {
   ...
   /**
    * The maximum age of a location retrieved from the passive location provider before it is considered too old to use when the
    * compass first starts up.
    */
   private static final long MAX_LOCATION_AGE_MILLIS  = TimeUnit.MINUTES.toMillis(30);

   /**
    * The sensors used by the compass are mounted in the movable arm on Glass. Depending on how this arm is rotated, it may
    * produce a displacement ranging anywhere from 0 to about 12 degrees. Since there is no way to know exactly how far the arm is
    * rotated, we just split the difference.
    */
   private static final int  ARM_DISPLACEMENT_DEGREES = 6;

   /**
    * Classes should implement this interface if they want to be notified of changes in the user's location, orientation, or the
   ...
   private final SensorEventListener    mSensorListener   = new SensorEventListener() {
                                                             ...
                                                             public void onSensorChanged(final SensorEvent event) {
                                                                if (event.sensor.getType() == Sensor.TYPE_ROTATION_VECTOR) {
                                                                   ...
                                                                   final float magneticHeading = (float) Math.toDegrees(mOrientation[0]);
                                                                   mHeading = MathUtils.mod(computeTrueNorth(magneticHeading),
                                                                            360.0f) - ARM_DISPLACEMENT_DEGREES;
   ...
   /**
    * Initializes a new instance of {@code OrientationManager}, using the specified context to access system services.
    */
   public OrientationManager(final SensorManager sensorManager,
    ...
    */
   public void addOnChangedListener(final OnChangedListener listener) {
    ...
    */
   public void removeOnChangedListener(final OnChangedListener listener) {
      mListeners.remove(listener);
   }


   /**
    * Starts tracking the user's location and orientation. After calling this method, any {@link OnChangedListener}s added to this
   ...
   public void start() {
      if (!mTracking) {
         ...
         final Location lastLocation = mLocationManager.getLastKnownLocation(LocationManager.PASSIVE_PROVIDER);
         if (lastLocation != null) {
            final long locationAge = lastLocation.getTime() - System.currentTimeMillis();
         ...
         if (mLocationProvider != null) {
            mLocationManager.requestLocationUpdates(mLocationProvider, MILLIS_BETWEEN_LOCATIONS, METERS_BETWEEN_LOCATIONS,
   ...
   /**
    * Stops tracking the user's location and orientation. Listeners will no longer be notified of these events.
    */
   public void stop() {
      if (mTracking) {
   ...
   /**
    * Gets the user's current heading, in degrees. The result is guaranteed to be between 0 and 360.
    * 
   ...
   /**
    * Gets the user's current pitch (head tilt angle), in degrees. The result is guaranteed to be between -90 and 90.
    * 
   ...
   private void notifyOrientationChanged() {
      for (final OnChangedListener listener : mListeners) {
   ...
   private void updateGeomagneticField() {
      mGeomagneticField = new GeomagneticField((float) mLocation.getLatitude(), (float) mLocation.getLongitude(),
               (float) mLocation.getAltitude(), mLocation.getTime());
   }
    ...
    */
   private float computeTrueNorth(final float heading) {
